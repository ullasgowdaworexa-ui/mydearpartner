...
import { Users, Plus, ToggleLeft, ToggleRight, Key, Trash2, Activity, Search } from 'lucide-react';
import {
  getAdminStaffList,
  createAdminStaff,
  updateAdminStaff,
...
export default function AdminStaffPage() {
  ...
  const basePath = isSuper ? '/super-admin/staff' : '/admin/staff';
  ...
  const [staffList, setStaffList] = useState<AdminAccount[]>([]);
  ...
  const fetchStaff = async () => {
    try {
      ...
      const data = await getAdminStaffList(params);
  ...
  useEffect(() => {
    if (matchCreate) {
      ...
      const found = staffList.find(s => s.id === editId);
      if (found) {
        openEdit(found);
      } else if (editId) {
        getAdminStaffList().then(res => {
      ...
      const found = staffList.find(s => s.id === detailId);
      if (found) {
        openEdit(found);
      }
    } else {
  ...
  }, [matchCreate, matchEdit, matchDetail, staffList.length, departments.length]);
  ...
  const handleToggleStatus = async (acc: AdminAccount) => {
    try {
      ...
      await actionAdminStaff(acc.id, action);
      fetchStaff();
    } catch (err: any) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      setError('');
      await actionAdminStaff(resetAccount.id, 'reset-password', {
  ...
  const openActivity = async (acc: AdminAccount) => {
    ...
    setActivityLogs([]);
    try {
      const logs = await getAdminStaffActivity(acc.id);
  ...
  return (
    <div className="admin-page-container" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: '700', color: '#f3f4f6', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <Users style={{ color: '#6366f1' }} /> Staff Management
          </h1>
          <p style={{ margin: '0.25rem 0 0', color: '#9ca3af' }}>Manage verification staff, review workloads, and audit logs.</p>
        </div>
        <button
                  ...
                  <td style={{ padding: '1rem' }}>
                    {acc.workload ? (
                      <div style={{ fontSize: '0.85rem' }}>
                        <span style={{ color: '#60a5fa' }}>A:{acc.workload.assigned}</span> |{' '}
                        <span style={{ color: '#fbbf24' }}>IP:{acc.workload.in_progress}</span> |{' '}
                        <span style={{ color: '#34d399' }}>C:{acc.workload.completed}</span>
                      </div>
                    ) : (
                      <span style={{ color: '#9ca3af' }}>â€”</span>
                    )}
      ...
      {modalOpen && (
        <div className="admin-dark-modal-backdrop">
          <div className="admin-dark-modal" style={{ maxWidth: 550 }}>
            <h2>
              {editing ? 'Edit Staff Member' : 'Add Staff Account'}
            </h2>
            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem' }}>
              ...
              <div style={{ gridColumn: 'span 2' }} className="admin-dark-modal-footer">
                <button type="button" className="admin-btn-cancel" onClick={() => setModalOpen(false)}>Cancel</button>
                <button type="submit" className="admin-btn-primary" disabled={submitting}>{submitting ? 'Savingâ€¦' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {resetAccount && (
        <div className="admin-dark-modal-backdrop">
          <div className="admin-dark-modal" style={{ maxWidth: 400 }}>
            <h2>Reset Password: {resetAccount.full_name}</h2>
            <form onSubmit={handleResetPassword}>
              <div className="admin-form-group">
                <label className="admin-form-label">New Secure Password</label>
                <input
                  type="password"
                  value={newPassword}
              ...
              <div className="admin-dark-modal-footer">
                <button type="button" className="admin-btn-cancel" onClick={() => setResetAccount(null)}>Cancel</button>
      ...
      {activityAccount && (
        <div className="admin-dark-modal-backdrop">
          <div className="admin-dark-modal" style={{ maxWidth: 650, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="admin-dark-modal-footer">
              <button type="button" className="admin-btn-cancel" onClick={() => setActivityAccount(null)}>Close Logs</button>
            </div>
          </div>
        </div>
      )}
